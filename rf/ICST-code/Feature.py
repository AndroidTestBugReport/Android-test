...
class FeatureClass:
    def __init__(self, root_Result, sameResultMList,similarResultMList, sameEvent, similarEventList):
        ...
        self.shortAbleList, self.editAbleList, self.editAbleIDList, self.longAbleList=self._extract()###divde all list into 2 parts as OneRunableFeature and oneEditAbleFeature
        #self._addBackMenu(self.runableList)
        
        self.wholeNum=len(self.shortAbleList)+len(self.editAbleList)+len(self.longAbleList)
    ...
    def _menuBackCases(self, oneRunable,runableEle):
    ...
    def _extract(self):
        ...
        for runableEle in runableEleList:
            ...
            matchCase=self._menuBackCases(oneRunable,runableEle)
            '''
            if matchCase:
                index+=1
                continue
            '''
            if matchCase:##ICST does not use this as feature
                continue 
            ...
            if "EditText" in viewName or "EditText" in childViewName:
                ...
                if clicktag=="long":
                    longAbleList.append(oneRunable)
                else:
                    shortAbleList.append(oneRunable)
            index+=1
        
        return shortAbleList, editAbleList, editAbleIDList, longAbleList
