...
def func(batch_size, featureListLen, Feature2Digit,word2VecModel,word2Idx, testLastTuple, recordMiddleList, agent, finishTag, sameBefore, iterCount, iteTime):
    print("lock acquire")
    lock.acquire()
    agent.replay(batch_size, featureListLen, Feature2Digit,word2VecModel,word2Idx, testLastTuple, recordMiddleList, sameBefore, iterCount, iteTime)
    finishTag[0]=1
    print("lock release")
    lock.release()
                            ...
                            if paral:
                                ...
                                while True:
                                    if finishTag[0]==1:
                                        finishTag=[0]
                                        break
                                t = threading.Thread(target=func, args=(batch_size, len(lastFeature.runableList),Feature2Digit,word2VecModel,word2Idx, testLastTuple, recordMiddleList, agent, finishTag, sameBefore, iterCount, iteTime)) 
                                t.start()
                            else:
                                
                                func(batch_size, len(lastFeature.runableList),Feature2Digit,word2VecModel,word2Idx, testLastTuple, recordMiddleList, agent, finishTag, sameBefore,iterCount, iteTime)
                    else:
